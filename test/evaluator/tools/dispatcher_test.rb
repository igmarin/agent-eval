...
module SkillBench
  module Tools
    # Tests for the SkillBench::Tools::Tools::Dispatcher
    class DispatcherTest < Minitest::Test
      def test_execute_read_file
        SkillBench::Tools::ReadFile.expects(:call).with('test.txt', anything).returns('file content')
      ...
      def test_execute_write_file
        SkillBench::Tools::WriteFile.expects(:call).with('new_file.txt', 'New text', anything).returns('success')
      ...
      def test_execute_run_command
        SkillBench::Tools::RunCommand.expects(:call).with('echo test', anything, nil).returns('command output')
      ...
      def test_execute_rescues_standard_error
        SkillBench::Tools::ReadFile.expects(:call).raises(StandardError, 'Oops')
        error = assert_raises(StandardError) do
